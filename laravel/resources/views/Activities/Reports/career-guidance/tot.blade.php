...
$('body').on('click', '.btn_view', function () {
      ...
      $.get("{{ url('reports-me/cg/tot') }}" +'/' + meeting_id +'/view', function (data) {
...
google.charts.load('current', {'packages':['corechart']});
      ...
      function drawChart1() {
        var data1 = google.visualization.arrayToDataTable([
          ...
          ['BEC', @if($bec->total_male==null) {{0}} @else {{$bec->total_male}} @endif , @if($bec->total_female==null) {{0}} @else {{$bec->total_female}}@endif, @if($bec->pwd_male==null) {{0}} @else{{$bec->pwd_male}}@endif,@if($bec->pwd_female==null) {{0}} @else{{$bec->pwd_female}}@endif],
          ['GVT officials', @if($gvt->total_male==null) {{0}} @else {{$gvt->total_male}} @endif , @if($gvt->total_female==null) {{0}} @else {{$gvt->total_female}}@endif, @if($gvt->pwd_male==null) {{0}} @else{{$gvt->pwd_male}}@endif,@if($gvt->pwd_female==null) {{0}} @else{{$gvt->pwd_female}}@endif],
          ['Gvt. Training Institues:',@if($gvt_TI->total_male==null) {{0}} @else {{$gvt_TI->total_male}} @endif , @if($gvt_TI->total_female==null) {{0}} @else {{$gvt_TI->total_female}}@endif, @if($gvt_TI->pwd_male==null) {{0}} @else{{$gvt_TI->pwd_male}}@endif,@if($gvt_TI->pwd_female==null) {{0}} @else{{$gvt_TI->pwd_female}}@endif],
          ['Pvt. Training Institues:', @if($pvt_TI->total_male==null) {{0}} @else {{$pvt_TI->total_male}} @endif , @if($pvt_TI->total_female==null) {{0}} @else {{$pvt_TI->total_female}}@endif, @if($pvt_TI->pwd_male==null) {{0}} @else{{$pvt_TI->pwd_male}}@endif,@if($pvt_TI->pwd_female==null) {{0}} @else{{$pvt_TI->pwd_female}}@endif]
        ]);
