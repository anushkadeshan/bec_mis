        ...
        <div class="container-fluid">
            <div class="panel panel-headline">
                <div class="panel-heading">
                    <h3 class="panel-title">Staff Field Visits</h3>
                </div>
                <div class="panel-body">
                    <table class="table table-hover" id="example">
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach($snapshot_sessions as $su)
                            <tr>
                                <td>{{$no++}}</td>
                                <td>{{$su->data()['date']}}</td>
                                <td>{{$su->data()['start_time']}}</td>
                                <td>{{$su->data()['end_time']}}</td>
                                <td>
                                    @php
                                ...
                                <td>{{$su->data()['client']}}</td>
                                <td>{{$su->data()['purpose']}}</td>
                                <td>
                                    <a href="{{url('firebase-sessions/view/'.$su->data()['uid'].'/'.$su->data()['id'].'')}}"><button class="btn btn-success btn-sm"><i class="lnr lnr-eye"></i></button></a>
                                </td>
                                <td>
                                    @if(isset($su->data()['updated_at']))
                                        {{date('Y-m-d | H:i:s',strtotime($su->data()['updated_at']))}}
                                    @endif</td>
                            </tr>
                            @endforeach
                        </tbody>
