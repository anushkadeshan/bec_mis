...
class FinancialSupportController extends Controller
{
                ...
                if ($number > 0) {
                    for ($i = 0; $i < $number; $i++) {
                        ...
                        if ($request->end_date > date("Y-m-d")) {
                            ...
                            DB::table('course_supports_youth')->where('youth_id', $request->youth_id[$i])->update(['current_status' => 2]);
                        }
                    }
                } else {
    ...
    public function add_youth(Request $request)
    {
        ...
        $participants = DB::table('finacial_supports_youths')
            ->insert(['youth_id' => $request->youth_id, 'approved_amount' => $request->approved_amount, 'installments' => $request->installments, 'finacial_support_id' => $request->m_id, 'current_status' => $current_status]);

        if ($request->edate > date("Y-m-d")) {
            DB::table('provide_soft_skills_youths')->where('youth_id', $request->youth_id)->update(['current_status' => 2]);
            DB::table('course_supports_youth')->where('youth_id', $request->youth_id)->update(['current_status' => 2]);
        }
        $youth_data = array(
            'added_by' => $added_by,
            'youth_count' => 1,
