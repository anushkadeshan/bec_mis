...
class SendMonthlyReport extends Command
{
    ...
    public function handle()
    {
        ...
        $cg = DB::table('cg_youths')
            ...
            ->whereBetween('career_guidances.program_date', [Carbon::today()->subMonths(1)->toDateTimeString(), Carbon::now()->toDateTimeString()])
            ->groupBy('branch_id')
            ->get();
        $data['cg'] = $cg;
        ...
        $soft = DB::table('provide_soft_skills_youths')
            ...
            ->whereBetween('provide_soft_skills.program_date', [Carbon::today()->subMonths(1)->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['soft'] = $soft;
        ...
        $vt = DB::table('finacial_supports_youths')
            ...
            ->whereBetween('finacial_supports.program_date', [Carbon::today()->subMonths(1)->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['vt'] = $vt;
        ...
        $gvt = DB::table('course_supports_youth')
            ...
            ->whereBetween('course_supports.program_date', [Carbon::today()->subMonths(1)->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['gvt'] = $gvt;
        ...
        $placement = DB::table('placement_individual')
            ...
            ->whereBetween('placement_individual.created_at', [Carbon::today()->subMonths(1)->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        ...
        foreach ($notifyTo as $notifyUser) {
            Mail::to($notifyUser->email)->send(new MonthlyReport($data));
        }
    }
}
