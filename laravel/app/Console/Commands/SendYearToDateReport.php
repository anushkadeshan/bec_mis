...
use Carbon\Carbon;
use App\User;
use App\Mail\YearToDate;
use Mail;

class SendYearToDateReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Report:Cumalative';
     ...
     * @var string
     */
    protected $description = 'Cumalative Report';
    ...
    public function handle()
    {
        $cg = DB::table('cg_youths')
            ...
            ->whereBetween('career_guidances.program_date', [Carbon::now()->startOfYear()->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['cg'] = $cg;
        ...
        $soft = DB::table('provide_soft_skills_youths')
            ...
            ->whereBetween('provide_soft_skills.program_date', [Carbon::now()->startOfYear()->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['soft'] = $soft;
        ...
        $vt = DB::table('finacial_supports_youths')
            ...
            ->whereBetween('finacial_supports.program_date', [Carbon::now()->startOfYear()->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['vt'] = $vt;
        ...
        $gvt = DB::table('course_supports_youth')
            ...
            ->whereBetween('course_supports.program_date', [Carbon::now()->startOfYear()->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['gvt'] = $gvt;
        ...
        $placement = DB::table('placement_individual')
            ...
            ->whereBetween('placement_individual.program_date', [Carbon::now()->startOfYear()->toDateString(), Carbon::now()->toDateString()])
            ->groupBy('branch_id')
            ->get();
        $data['placement'] = $placement;
        $data['date1'] = Carbon::now()->startOfYear()->toDateString();
        $data['date2'] = Carbon::now()->toDateString();
        // dd($data);
        $notifyTo = User::whereHas('roles', function ($q) {
            $q->whereIn('slug', ['admin', 'management', 'branch']);
        })->get();
        foreach ($notifyTo as $notifyUser) {
            Mail::to($notifyUser->email)->send(new YearToDate($data));
        }
    }
}
